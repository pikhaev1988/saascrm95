"""
Синхронизация тем ЕГЭ по математике (базовый уровень) из Excel
в data/ege_2026_enriched.json.
Запуск: python scripts/sync_math_basic_topics_from_xlsx.py
"""
...
XLSX = Path.home() / "Desktop" / "Новая папка (5)" / "математика базовый.xlsx"
...
_LOWER_GRADE_RE = re.compile(r"(?:^|\s)[5-9]\s*(?:кл\.|класс)", flags=re.I)
...
def main() -> None:
    ...
    math_basic = next((subject for subject in subjects if subject.get("subject") == "math_basic"), None)
    if not math_basic:
        raise SystemExit("В JSON нет subject=math_basic")

    old_by_task = {int(task["task"]): task for task in math_basic.get("tasks", []) if task.get("task") is not None}
    ...
    math_basic["tasks"] = merged_tasks
    ENRICHED.write_text(json.dumps(data, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"OK: обновлено {len(merged_tasks)} заданий по математике (базовой) в {ENRICHED}")
