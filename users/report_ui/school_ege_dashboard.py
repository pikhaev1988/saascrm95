"""
Presentation packaging for school EGE analytics dashboard.

Pass rates use official EgePassingThreshold (year fallback),
not the import-time ExamResult.passed flag.
Quality ≥ 60 and high ≥ 70 remain display thresholds.
"""
...
def build_ege_dashboard_ui(
    *,
    ...
    year_qs = year_qs if year_qs is not None else None
    ...
    if year_qs is not None:
        ...
        agg = year_qs.aggregate(
            avg=Avg("score"),
            quality=Count("id", filter=Q(score__gte=60)),
            high=Count("id", filter=Q(score__gte=70)),
        )
        ...
        enriched_subjects.append(
            {
                **item,
                ...
                "tone": tone,
                "level_pct": min(100, max(0, float(pr if pr is not None else (avg or 0)))),
            }
        )

    # Subject-level KPIs (selected exam) — from analysis / selected rows
...
def _subject_kpi(analysis: dict, selected_results: list | None) -> dict:
    ...
    if rows:
        ...
        # Пересчёт успеваемости по порогу из строк результата (below_minimum).
    ...
    bars = [
        {
            ...
            "value": subject_avg,
            "pct": _score_to_pct(subject_avg),
            "tone": "blue",
        },
        {
            "label": "Относительно школы (среднее по срезу)",
            "value": school_avg,
            "pct": _score_to_pct(school_avg),
            "tone": "navy",
        },
    ]
    # Year trend from comparison if present
        ...
        bars.append(
            {
                ...
                "value": district_avg,
                "pct": _score_to_pct(district_avg),
                "tone": "mid",
            }
        )
    else:
        ...
        bars.append(
            {
                ...
                "value": republic_avg,
                "pct": _score_to_pct(republic_avg),
                "tone": "mid",
            }
        )
    else:
...
def _delta_to_pct(v) -> float:
    try:
        # map delta roughly into 0..100 for bar width (0 → 50)
        return max(0.0, min(100.0, 50.0 + float(v) * 2))
    except (TypeError, ValueError):
        return 50.0
